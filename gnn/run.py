...
parser.add_argument('--model', type=str, default='model_gat.pkl')
...
parser.add_argument('--mode', choices=["att", "concat", "mid", "low", "sage"], help='the version of models', default='att')
...
if __name__ == '__main__':
    ...
    for run in range(args.runs):
        ...
        # model = SAGEConv(args.embedding_dim, 20)
        ...
        for epoch in range(args.num_epoch):
            ...
            if 1:
                if epoch % args.val_epoch == 0:
                    with torch.no_grad():
                        model.eval()
                        hr5_score, hr10_score, ndcg_score, _, _= model.inference(features, price_bin, adj, val_set)
                    ...
                    if ndcg_score > best_ndcg:
                        ...
                        best_epoch = epoch
                        cnt_wait = 0
                        torch.save(model.state_dict(), '{}/'.format(ckpt_path) + args.model)
                    else:
                        cnt_wait += 1
        ...
        if 1:
            ...
            model.load_state_dict(torch.load('{}/'.format(ckpt_path) + args.model))
            print('Testing AUC!', flush=True)
            with torch.no_grad():
                model.eval()
                hr5_score, hr10_score, ndcg_score, _, _= model.inference(features, price_bin, adj, test_set)
                hr5_score = hr5_score.to(torch.device('cpu'))
                hr10_score = hr10_score.to(torch.device('cpu'))
