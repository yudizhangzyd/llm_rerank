...
class SComGNN(nn.Module):
    ...
    def __init__(self, embedding_size, price_n_bins, mode, category_emb_size=768):
        ...
        self.item_gc = GATConv(embedding_size, 128, 2, dropout=0.6)
        # self.item_gc = SAGEConv(embedding_size, embedding_size)
        # mlp = Sequential(
        #         Linear(embedding_size, 2 * 128),
        #         BatchNorm(2 * 128),
        #         ReLU(),
        #         Linear(2 * 128, 128),
        #     )
        # self.item_gc = GINConv(mlp, train_eps=True)
    ...
    def inference(self, features, price, adj, test_set):
        ...
        hr5, hr10, ndcg, scores, labels = self.metrics(torch.unsqueeze(pos_scores, 1), neg_scores)

        return hr5, hr10, ndcg, scores, labels
    ...
    def metrics(self, pos_scores, neg_scores):
        ...
        return hr_list[0], hr_list[1], ndcg, scores, labels